...
for day in "${days[@]}"; do
	...
	actual_output_dir="weekly/${day}/actual_output"
	expected_output_dir="weekly/${day}/expected_output"
	...
	./daily.sh "weekly/${day}/inputs" "$actual_output_dir" "$expected_output_dir" "$accounts_file" "$collection_file" "$available_games_file"
done
