...
if [ $? -eq 0 ]; then
    ...
    ../../project/main < "../../test_cases/refund/TCR9 - Buyer Game Availability (fail)/TCR9.in.txt" > "../../scripts/refund/outputTCR9.txt"

    # Check if program execution was successful
    if [ $? -eq 0 ]; then
        echo "Program executed successfully."
        # Compare output with expected output
        diff -q "../../scripts/refund/outputTCR9.txt" "../../test_cases/refund/TCR9 - Buyer Game Availability (fail)/TCR9.out.txt"
        if [ $? -eq 0 ]; then
