...
if [ $? -eq 0 ]; then
    ...
    ../../project/main < "../../test_cases/refund/TCR14 - Transaction Code Validity (success)/TCR14.in.txt" > "../../scripts/refund/outputTCR14.txt"

    # Check if program execution was successful
    if [ $? -eq 0 ]; then
        echo "Program executed successfully."
        # Compare output with expected output
        diff -q "../../scripts/refund/outputTCR14.txt" "../../test_cases/refund/TCR14 - Transaction Code Validity (success)/TCR14.out.txt"
        if [ $? -eq 0 ]; then
