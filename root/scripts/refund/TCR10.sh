...
if [ $? -eq 0 ]; then
    ...
    ../../project/main < "../../test_cases/refund/TCR10 - Transaction Code Validity (fail)/TCR10.in.txt" > "../../scripts/refund/outputTCR10.txt"

    # Check if program execution was successful
    if [ $? -eq 0 ]; then
        echo "Program executed successfully."
        # Compare output with expected output
        diff -q "../../scripts/refund/outputTCR10.txt" "../../test_cases/refund/TCR10 - Transaction Code Validity (fail)/TCR10.out.txt"
        if [ $? -eq 0 ]; then
