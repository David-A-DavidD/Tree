...
if [ $? -eq 0 ]; then
    ...
    ../../project/main < "../../test_cases/refund/TCR11 - Username Validity (Buyer) (success)/TCR11.in.txt" > "../../scripts/refund/outputTCR11.txt"

    # Check if program execution was successful
    if [ $? -eq 0 ]; then
        echo "Program executed successfully."
        # Compare output with expected output
        diff -q "../../scripts/refund/outputTCR11.txt" "../../test_cases/refund/TCR11 - Username Validity (Buyer) (success)/TCR11.out.txt"
        if [ $? -eq 0 ]; then
