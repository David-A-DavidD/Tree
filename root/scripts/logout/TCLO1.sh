...
if [ $? -eq 0 ]; then
    ...
    ../../project/main < "../../test_cases/logout/TCLO1 - Checking if User is Logged In (fail)/TCLO1.in.txt" > "../../scripts/logout/outputTCLO1.txt"

    # Check if program execution was successful
    if [ $? -eq 0 ]; then
        echo "Program executed successfully."
        # Compare output with expected output
        diff -q "../../scripts/logout/outputTCLO1.txt" "../../test_cases/logout/TCLO1 - Checking if User is Logged In (fail)/TCLO1.out.txt"
        if [ $? -eq 0 ]; then
