...
if [ $? -eq 0 ]; then
    ...
    ../../project/main < "../../test_cases/create/TCCR2 - username threshold character check /TCCR2.in.txt" > "../../scripts/create/outputTCCR2.txt"

    # Check if program execution was successful
    if [ $? -eq 0 ]; then
        echo "Program executed successfully."
        # Compare output with expected output
        diff -q "../../scripts/create/outputTCCR1.txt" "../../test_cases/create/TCCR2 - username threshold character check/TCCR2.out.txt"
        if [ $? -eq 0 ]; then
